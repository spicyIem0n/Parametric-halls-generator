"""
RoofFactory — generuje dźwigary kratowe, płatwie i poszycie dachowe.
...
class RoofFactory:
    ...
    def generate(grid: GridSystem3D, params: HallParameters) -> list[Component3D]:
        ...
        col_w = grid.col_width
        wall_t = params.cladding_thickness
        chord_t = DEFAULTS.truss_chord_thickness
        web_t = DEFAULTS.truss_web_thickness
        purlin_chord_t = DEFAULTS.purlin_chord_thickness
        purlin_web_t = DEFAULTS.purlin_web_thickness

        # Wysunięcie dachu do lica zewnętrznego
        ...
        if params.roof_drainage_type == "gravity":
            ...
            zone_w = grid.width / params.drainage_zones_x
            zone_l = grid.length / params.drainage_zones_z
            ...
            for ix in range(params.drainage_zones_x):
                for iz in range(params.drainage_zones_z):
                    ...
                    inlet_z = -grid.half_length + (iz * zone_l) + (zone_l / 2)
