...
def _build_wall_with_hole(x, z_center, bay_len, t, parapet_h, hole_w, hole_h, hole_y_start, orientation="horizontal"):
    """Zestawia ścianę z paneli omijając otwór na dok/bramę."""
    pieces = []
    m = {"cladding_orientation": orientation}
    # 1. Lewy i prawy panel boczny
    side_w = (bay_len - hole_w) / 2
    if side_w > 0:
        pieces.append(Component3D(type="sandwich_panel", position=[x, parapet_h / 2, z_center - bay_len / 2 + side_w / 2], rotation=[0, 0, 0], scale=[t, parapet_h, side_w], meta=dict(m)))
        pieces.append(Component3D(type="sandwich_panel", position=[x, parapet_h / 2, z_center + bay_len / 2 - side_w / 2], rotation=[0, 0, 0], scale=[t, parapet_h, side_w], meta=dict(m)))
    ...
    if top_h > 0:
        pieces.append(Component3D(type="sandwich_panel", position=[x, hole_top + top_h / 2, z_center], rotation=[0, 0, 0], scale=[t, top_h, hole_w], meta=dict(m)))

    # 3. Dolny panel (pod dokiem)
    if hole_y_start > 0:
        pieces.append(Component3D(type="sandwich_panel", position=[x, hole_y_start / 2, z_center], rotation=[0, 0, 0], scale=[t, hole_y_start, hole_w], meta=dict(m)))
...
class CladdingFactory:
    ...
    def generate(grid: GridSystem3D, params: HallParameters) -> list[Component3D]:
        ...
        orient = getattr(params, "cladding_orientation", "horizontal")
        cl_meta = {"cladding_orientation": orient}
